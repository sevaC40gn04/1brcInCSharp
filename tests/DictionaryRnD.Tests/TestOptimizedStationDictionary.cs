...
public class TestOptimizedStationDictionary
{
    public struct Entry
    {
    ...
    public TestOptimizedStationDictionary(int lenCapacity)
    {
    ...
    public unsafe ref Entry AddGet(TestOptStationKey key)
    {
        ...
        CollisionCount = key.CollisionCount;
        return ref returnedEntry;
    }

    public unsafe TestOptStationKey GenerateKey(byte[] buffer)
    {
        byte* bytePtr;
        uint* pUint32;
        ulong* pUint64;
    ...
    public unsafe List<TestStationResult> GetSortedCalculatedList()
    {
        var totalCount = 0;
    ...
    delegate void CopyName(ref Entry srcEntry, TestStationResult stationResult);
    ...
    private static unsafe void BuildNameFromHash_4_1(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUintPtr = srcEntry.Hash_4_1;
        stationResult.Name = Encoding.UTF8.GetString(convertToNameBuffer, 0, srcEntry.NameLen);
    }
    private static unsafe void BuildNameFromHash_8_1(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        stationResult.Name = Encoding.UTF8.GetString(convertToNameBuffer, 0, srcEntry.NameLen);
    }
    private static unsafe void BuildNameFromHash_8_1_4_1(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        *ctnbUint8Ptr = srcEntry.Hash_4_1;
        stationResult.Name = Encoding.UTF8.GetString(convertToNameBuffer, 0, srcEntry.NameLen);
    }
    private static unsafe void BuildNameFromHash_8_1_8_2(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        *ctnbUlong8Ptr = srcEntry.Hash_8_2;
        stationResult.Name = Encoding.UTF8.GetString(convertToNameBuffer, 0, srcEntry.NameLen);
    }
    private static unsafe void BuildNameFromHash_8_1_8_2_4_1(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        *ctnbUlong8Ptr = srcEntry.Hash_8_2;
    ...
    private static unsafe void BuildNameFromHash_8_1_8_2_8_3(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        *ctnbUlong8Ptr = srcEntry.Hash_8_2;
    ...
    private static unsafe void BuildNameFromHash_8_1_8_2_8_3_4_1(ref Entry srcEntry, TestStationResult stationResult)
    {
        *ctnbUlongPtr = srcEntry.Hash_8_1;
        *ctnbUlong8Ptr = srcEntry.Hash_8_2;
        *ctnbUlong16Ptr = srcEntry.Hash_8_3;
    ...
    public void AddDictionary(TestOptimizedStationDictionary srcDictionary)
    {
    ...
    delegate ref Entry SetHash(TestOptStationKey key, ref int count);
    ...
    private static unsafe ref Entry ProcessLen1(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
    ...
    private static unsafe ref Entry ProcessLen2(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen3(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen4(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen5(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen6(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    public static unsafe ref Entry ProcessLen7(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen8(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen9(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen10(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen11(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen12(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen13(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen14(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen15(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen16(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen17(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen18(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen19(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen20(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen21(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen22(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen23(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen24(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen25(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen26(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen27(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
    ...
    private static unsafe ref Entry ProcessLen28(TestOptStationKey key, ref int count)
    {
        var nameLen = key.NameLen;
        var usedBucket = key.buckets[nameLen];
        var usedEntries = key.entries[nameLen];
...
public class TestOptStationKey
{
    ...
    public int[][] buckets;
    public TestOptimizedStationDictionary.Entry[][] entries;
...
public class TestStationResult
{
    public string Name;
